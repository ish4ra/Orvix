...
class AiSinhalaSubtitleService {
  ...
  static Future<_EmbeddedSubtitleSource?> _fetchLocalP2pEmbeddedEnglishSubtitle(
    ...
    for (final entry in raw) {
      ...
      // The exact-file stream-server also reports subtitle files that are
      // separate torrent files (.srt/.ass/.vtt/etc.). beta.27 discarded them
      // because they correctly have embedded=false. Accept one only when its
      // filename/path strongly identifies the selected episode/movie release;
      // its actual dialogue is verified as English after download.
      if (!embedded &&
    ...
    // External subtitle files are tiny. Try only the strongest exact-release
    // candidates so a season pack cannot turn unrelated subtitle files into a
    // long startup chain.
    for (final candidate in externalCandidates.take(4)) {
      ...
      try {
        ...
        return _EmbeddedSubtitleSource(
          content: content,
          identity: 'orvix-torrent-external://' +
              identity.infoHash +
              '/' +
              (candidate['id'] ?? relativeUrl).toString(),
  ...
  static String? _releaseEpisodeKey(String raw) {
    ...
    final compact =
        RegExp(r'(^|[^0-9])(\d{1,2})x(\d{1,3})([^0-9]|$)').firstMatch(lower);
    if (compact != null) {
  ...
  static int _externalTorrentSubtitleMatchScore(
    ...
    // A series/season-pack subtitle must identify the same episode explicitly.
    // Ambiguous "English.srt" files are never auto-attached across a pack.
