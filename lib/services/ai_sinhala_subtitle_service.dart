...
class AiSinhalaSubtitleService {
  ...
  static Future<_EmbeddedSubtitleSource?> _fetchEmbeddedEnglishSubtitle(
    ...
    if (localP2p) {
      ...
      // Windows ships the exact-file Orvix stream-server extension. Once that
      // server has answered the exact torrent/file query, re-running FFmpegKit
      // against the same localhost torrent URL only duplicates the work and
      // can add 40+ seconds to a failed preflight. Other platforms retain the
      // generic FFmpegKit fallback because they may use an unmodified engine.
  ...
  static Future<_EmbeddedSubtitleSource?> _fetchLocalP2pEmbeddedEnglishSubtitle(
    ...
    final embeddedCandidates = <Map<String, dynamic>>[];
    final externalCandidates = <Map<String, dynamic>>[];
    for (final entry in raw) {
      if (entry is! Map) continue;
      final map = Map<String, dynamic>.from(entry);
      ...
      if (embedded && url.contains('/embedded/')) {
        final score = _embeddedEnglishTrackScore(
          label,
          preferredTrackLabel: preferredTrackLabel,
        );
        if (score <= 0) continue;
        map['_score'] = score;
        embeddedCandidates.add(map);
        continue;
      }

      // The modified stream-server also returns real subtitle files contained
      // in the torrent (.srt/.ass/.vtt/etc.). beta.27 discarded every one of
      // them because they are correctly marked embedded=false. Accept one only
      // when its filename/path strongly identifies the same selected episode
      // or movie release. The subtitle text is independently checked as English
      // after download before it can become an AI timing source.
      if (!embedded &&
          !url.contains('/embedded/') &&
          videoFileNameHint?.trim().isNotEmpty == true) {
        final score = _externalTorrentSubtitleMatchScore(
          label,
          videoFileNameHint!,
        );
        if (score > 0) {
          map['_score'] = score;
          externalCandidates.add(map);
        }
      }
    }

    embeddedCandidates.sort(
      (a, b) => (b['_score'] as int).compareTo(a['_score'] as int),
    );
    ...
    for (final candidate in embeddedCandidates) {
    ...
    // External subtitle files are tiny compared with the video. Try only the
    // strongest release/episode matches so a season pack cannot turn one slow
    // or unrelated .srt into another multi-minute startup path.
    for (final candidate in externalCandidates.take(4)) {
      final relativeUrl = candidate['url']?.toString() ?? '';
      final label = candidate['label']?.toString() ?? 'English external';
      final subtitleUri = videoUri.resolve(relativeUrl);
      try {
        final response = await http
            .get(subtitleUri)
        ...
        final content =
            utf8.decode(response.bodyBytes, allowMalformed: true).trim();
        if (content.isEmpty || _parseSubtitle(content).length < 8) continue;
        if (!_subtitleTextLooksEnglish(content)) continue;
        return _EmbeddedSubtitleSource(
          content: content,
          identity:
              'orvix-torrent-external://${identity.infoHash}/${candidate['id'] ?? relativeUrl}',
          label: 'Torrent external • $label',
        );
      } catch (_) {
        continue;
      }
  ...
  static String? _releaseEpisodeKey(String raw) {
    final lower = raw.toLowerCase();
    final standard =
        RegExp(r's(\d{1,2})[ ._-]*e(\d{1,3})').firstMatch(lower);
    if (standard != null) {
    ...
    final compact = RegExp(r'(^|[^0-9])(\d{1,2})x(\d{1,3})([^0-9]|$)')
        .firstMatch(lower);
    if (compact != null) {
  ...
  static Set<String> _releaseFileTokens(String raw) {
    final value = raw
        .toLowerCase()
        .replaceAll('\\', '/')
        .replaceAll(RegExp(
          r'\.(?:srt|ass|ssa|vtt|sub|txt|mkv|mp4|avi|webm|mov)
...
,
    ...
    const noise = <String>{
      'en', 'eng', 'english', 'sub', 'subs', 'subtitle', 'subtitles',
      'sdh', 'cc', 'hi', 'forced', 'utf8', 'utf', 'default',
    };
    return value
        .split(RegExp(r'\s+'))
  ...
  static int _externalTorrentSubtitleMatchScore(
    String subtitleLabel,
    String videoFileNameHint,
  ) {
    final subtitleEpisode = _releaseEpisodeKey(subtitleLabel);
    final videoEpisode = _releaseEpisodeKey(videoFileNameHint);

    // For series/season packs an explicit episode identity is mandatory. A
    // subtitle with no SxxEyy/1xEE marker is too ambiguous to auto-attach.
    if (videoEpisode != null) {
      if (subtitleEpisode == null || subtitleEpisode != videoEpisode) return -1;
    } else if (subtitleEpisode != null) {
      return -1;
    }

    final subtitleTokens = _releaseFileTokens(subtitleLabel);
    final videoTokens = _releaseFileTokens(videoFileNameHint);
    if (subtitleTokens.isEmpty || videoTokens.isEmpty) return -1;

    final overlap = subtitleTokens.intersection(videoTokens).length;
    final shorter = math.min(subtitleTokens.length, videoTokens.length);
    final ratio = shorter == 0 ? 0.0 : overlap / shorter;

    // Same-episode files normally share the title/release stem. Movies have no
    // episode key, so demand an even stronger filename relationship.
    if (videoEpisode != null) {
      if (overlap < 2 || ratio < .35) return -1;
    } else {
      if (overlap < 3 || ratio < .55) return -1;
    }
  ...
  static bool _subtitleTextLooksEnglish(String content) {
    final cues = _parseSubtitle(content);
    if (cues.length < 8) return false;
    final sample =
        cues.take(120).map((cue) => cue.source).join(' ').toLowerCase();
    ...
    const common = <String>{
      'the', 'and', 'you', 'that', 'this', 'with', 'have', 'what', 'for',
      'not', 'are', 'your', 'but', 'from', 'they', 'will', 'just', 'can',
      'was', 'there', 'here', 'about', 'know', 'like', 'want', 'get', 'got',
      'we', 'he', 'she', 'his', 'her', 'our', 'out', 'one', 'all', 'why',
      'how', 'when', 'where', 'who', 'would', 'could', 'should',
    };
